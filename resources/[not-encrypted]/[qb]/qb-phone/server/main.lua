...
local WebHook = "https://discord.com/api/webhooks/978246651103838248/yCIdU9i3tnXKy-3ExlX8-WydCoJBXzh4ox7TQnVSTuwZLbJypsvM-yiKkCPS-Yf-5WTH"
...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player then
        ...
        if images ~= nil and next(images) then
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    TriggerEvent('qb-banking:society:server:DepositMoney', source, amount , soiety)
...
QBCore.Functions.CreateCallback('qb-phone:server:FetchResult', function(source, cb, search)
    ...
    if result[1] then
        for k, v in pairs(result) do
            ...
            local appiepappie = {}
            if ApaData[v.citizenid] ~= nil and next(ApaData[v.citizenid]) then
...
RegisterNetEvent('qb-phone:server:UpdateHashtags', function(Handle, messageData)
    if Hashtags[Handle] ~= nil and next(Hashtags[Handle]) then
