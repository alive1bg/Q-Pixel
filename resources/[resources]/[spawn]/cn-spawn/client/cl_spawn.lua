...
RegisterNetEvent("character:PlayerSelectedCharacter", function(charId)
	SendNUIMessage({
		close = true
	})
...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = QBCore.Functions.ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		QBCore.Functions.TriggerCallback("cn-spawn:deleteCharacter", function(deleted) 
			Login.getCharacters(true)
		end, data.actionData)
	end
	...
	if data.action == "newCharacter" then
		...
		local sex = 0
		if cData.sex == "f" then sex = 1 Login.secilensex = 1 end
		local chardata = {
			cid = cData.cid,
		...
		QBCore.Functions.TriggerCallback("cn-spawn:createCharacter", function(created) 
			if not created then
			...
			if type(created) == "table" and created.err then
				QBCore.Functions.Notify(created.msg)
				Login.CreatePlayerCharacterPeds(Login.CurrentClothing, true)
                return
			end
...
RegisterNetEvent("PlayerSpawned2", function()
	Login.nativeStart()
end)
