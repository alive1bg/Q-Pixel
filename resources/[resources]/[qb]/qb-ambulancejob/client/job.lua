...
RegisterNetEvent('custom_ambulance:playCPR')
AddEventHandler('custom_ambulance:playCPR', function(playerheading, playercoords, playerlocation)
    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                TriggerServerEvent('custom_ambulance:requestCPR', GetPlayerServerId(player), GetEntityHeading(playerPed), GetEntityCoords(playerPed), GetEntityForwardVector(playerPed))
                QBCore.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), 5000, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    ...
                    isHealingPerson = false
                    StopAnimTask(playerPed, healAnimDict, "exit", 1.0)
                    QBCore.Functions.Notify(Lang:t('error.cancled'), "error")
                end)
            else
                QBCore.Functions.Notify(Lang:t('error.no_player'), "error")
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if PlayerJob.name =="ambulance" and onduty then
               --[[ for k, v in pairs(Config.Locations["duty"]) do
                ...
                end ]]-- -- ARMORY MARKER AND DRAWTEXT
