...
createRulettAsztal = function(index, data)
    ...
    RequestModel(GetHashKey('vw_prop_casino_roulette_01b'))
    while not HasModelLoaded(GetHashKey('vw_prop_casino_roulette_01b')) do
        Wait(1)
    end

    self.tableObject = CreateObject(GetHashKey('vw_prop_casino_roulette_01b'), data.position, false)
...
function casinoNuiUpdateGame(rulettIndex, ido, statusz)
    QBCore.Functions.TriggerCallback('BLACKJACK:server:blueChipsAmount', function(result)
        ...
        if selectedRulett == rulettIndex then
            if not statusz then
                exports['qb-casinoui']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>"..ido.." Seconds Left</p>Current Bet: "..currentBetAmount.." chips</p>Availble chips: "..retval)
                if Config.allowCustomBet then
