...
AddEventHandler('mrp-paychecks:Register', function()
    ...
    if xPlayer.PlayerData.job.onduty then
        exports.oxmysql:query("SELECT * FROM `paychecks` WHERE citizenid = '"..cid.."'", function(result)
            if result[1] ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Your paycheck has been sent to the bank! Head there to collect it at your convenience.')
                Wait(1000)
...
AddEventHandler('mrp-paychecks:Collect', function()
    ...
    exports.oxmysql:query("SELECT * FROM `paychecks` WHERE citizenid = '"..cid.."'", function(result)
        if result[1] ~= nil then
            ...
            -- print('You don\'t have a job or your paycheck isnt ready?')
            TriggerClientEvent('QBCore:Notify', source, 'You don\'t have a job or your paycheck isnt ready?')
        end
    end)
end)