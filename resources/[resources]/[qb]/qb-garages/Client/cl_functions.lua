...
function SpawnVehicle(vehicle, pGarage, Fuel, body, engine, plate, gType, IsViewing, IsShared)
    ...
    if  ParkingSpot ~= nil then
        ...
        QBCore.Functions.Notify('Need some Space. Move a bit further!', 'error')
    end
end
                ...
                if Var.DepotGetDestroyedVeh then
                    ...
                    if Engine < 40 then
                        if spawnpoint ~= nil then
                            QBCore.Functions.TriggerCallback('qb-garage:server:PayDepotPrice', function(paid)
                                if paid then
                                    if not IsModelInCdimage(model) then return end
                                    RequestModel(model)
                                    while not HasModelLoaded(model) do Citizen.Wait(10) end
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                                        ...
                                        exports['qb-fuel']:SetFuel(veh, Data.fuel)
                            ...
                            QBCore.Functions.Notify('Unable to find free space!', 'error')
                        end
                    else
                        QBCore.Functions.Notify('You can not claim undestroyed vehicles.', 'error', 1000)
...
function AddTemporaryBlip(vehicle)  
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Car: "..QBCore.Shared.Vehicles[vehname].name)
    EndTextCommandSetBlipName(TempBlip)
    QBCore.Functions.Notify("Your Car "..QBCore.Shared.Vehicles[vehname].name.." is indicated on the map for 1 minute!", "success", 10000)
    SetTimeout(60 * 1000, function()
        TriggerEvent('QBCore:Notify', 'Your vehicle is no longer displayed on the map!', 'error')
        RemoveBlip(TempBlip)
    end)
end
