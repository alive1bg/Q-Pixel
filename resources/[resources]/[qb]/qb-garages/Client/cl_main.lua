...
RegisterNetEvent('Garages:Store', function()
    ...
    QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
        ...
        if owned then
            ...
            local pGarage = GetJobGarage()
            QBCore.Functions.TriggerCallback('qb-garage:server:isSharedVehicle', function(isShared)
                if isShared then
                    if pJob.name == JobGarages[pGarage].job then
...
RegisterNetEvent('qb-garages:VehicleList', function()
    ...
    local pSpot = GetpSpot(pGarage)
    QBCore.Functions.TriggerCallback('qb-garages:server:GetPlayerVehicles', function(vehcheck)
...
RegisterNetEvent('qb-garages:client:DepotVehicleList', function()
    QBCore.Functions.TriggerCallback('qb-garages:server:GetDepotVehicles', function(vehcheck)
        local NoVeh = false
        local menu = {{
            header = "< Go Back",
            params = {
...
RegisterNetEvent('qb-garages:client:SharedVehicleMenu', function()
    ...
    if pJob.onduty then
        QBCore.Functions.TriggerCallback('qb-garage:server:CheckSharedCategories', function(result)
            if result ~= nil then
                local categories = {}
                for i = 1, #result do
...
RegisterNetEvent('qb-garages:client:SharedHeliGarage', function()
    DeleteViewedCar()
    local pGarage = GetJobGarage()
    QBCore.Functions.TriggerCallback('qb-garages:server:GetSharedHeli', function(vehcheck)
        if vehcheck ~= nil then
            local isLocked = false
            local menu = {{
                header = "< Close Menu",
...
RegisterNetEvent('qb-garages:client:SharedVehicleList', function(Data)
    DeleteViewedCar()
    local pGarage = GetJobGarage()
    QBCore.Functions.TriggerCallback('qb-garages:server:GetSharedVehicles', function(vehcheck)
        if vehcheck ~= nil then
            local isLocked = false
            local menu = {{
                header = "< Go Back",
...
RegisterNetEvent('qb-garages:client:ParkingLog', function(Data)
    ...
    QBCore.Functions.TriggerCallback('qb-garage:server:GetParkingLog', function(plog)
        if plog ~= nil then
            for i = 1, #plog do
                table.insert(menu, {
...
RegisterNetEvent('qb-garages:client:AddSharedVehicle', function(garage, faction, category)
    if JobGarages[garage] ~= nil then
        ...
        if curVeh ~= 0 then
            if model ~= nil then
                QBCore.Functions.TriggerCallback('qb-garage:server:isVehicleOwned', function(owned)
                    if owned then
                        QBCore.Functions.Notify("You Can't save player owned vehicles!", "error")
                    else
                        QBCore.Functions.TriggerCallback('qb-garage:server:isVehicleShared', function(shared)
                            if shared then
