...
RegisterNetEvent('qb-postalop:startwork', function()
	if not isDeliverySignedIn and not onJob then	
		if not DoesEntityExist(goPostalVehicle) then 
			if QBCore.Functions.GetPlayerData().money.cash >= 1000 then
				...
				TriggerServerEvent('qb-delivery:cash', 1000, "remove")	  
				NewDeliveryShift()						
				onJob = true
...
RegisterNetEvent('qb-postalop:takepackage', function()
...
RegisterNetEvent('qb-postalop:finishwork', function()
	if isDeliverySignedIn then	
		if not onJob then
			if DoesEntityExist(goPostalVehicle) then 																												
				...
				if IsVehicleDamaged(goPostalVehicle) then
					TriggerServerEvent('qb-delivery:cash', 200, "add")
				else
					TriggerServerEvent('qb-delivery:cash', 1000, "add")
				end	  							
				isDeliverySignedIn = false	
				onJob = false	
				TriggerServerEvent('qb-delivery:cash', totalpayamount, "job")	
				Wait(500)
				totalpayamount = 0
			else
				isDeliverySignedIn = false	
