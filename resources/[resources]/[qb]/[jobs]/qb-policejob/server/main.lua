...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        if (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "ambulance" or v.PlayerData.job.name == "bcso" or v.PlayerData.job.name == "sasp" or v.PlayerData.job.name == "saspr" or v.PlayerData.job.name == "sdso" or v.PlayerData.job.name == "pbso") and v.PlayerData.job.onduty then
...
QBCore.Commands.Add("plateinfo", Lang:t("commands.plateinfo"), {{name = "plate", help = Lang:t('info.plate_number')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" --[[and Player.PlayerData.job.onduty]] then
        if not IsVehicleOwned(plate) then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Its owned by Civilian", 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
...
-- QBCore.Commands.Add("impound", Lang:t("commands.impound"), {}, false, function(source) -- commented out for now whilst doing impound shit
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
--     if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer then
            ...
            TriggerEvent('qb-management:server:addAccountMoney', "police", price)
...
RegisterNetEvent("qb-policejob:server:AddTheftPlate", function(plate)
    ...
    if Player.PlayerData.job.name ~= "police" then
        ...
        if math.random(1, 100) < chance then
            local result = MySQL.single('SELECT citizenid FROM player_vehicles WHERE plate = @plate', {
                ['@plate'] = plate
            })
            if not result then
                Plates[plate] = {
                    isflagged = true,
                    reason = "Сигнал за откраднат!"
                }
            end
        end
    end
