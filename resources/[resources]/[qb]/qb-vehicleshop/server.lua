...
QBCore.Functions.CreateCallback('qb-vehicleshop:server:getVehicles', function(source, cb)
    ...
    if player then
        ...
        if vehicles[1] then
            ...
            TriggerClientEvent('QBCore:Notify', src, Lang:t('error.novehicles'), 'error')
        end
    end
end)
...
RegisterNetEvent('qb-vehicleshop:server:checkFinance', function()
    ...
    if result[1] then
        ...
        for _, v in pairs(vehicles) do
            ...
            MySQL.update('UPDATE player_vehicles SET state = ?, depotprice = ? WHERE plate = ?', {0, v.balance * 1.5, plate,}) -- Use this if you don't want them to be deleted
