...
QBCore.Functions.CreateUseableItem('policegunrack', function(source, item)
     ...
     TriggerClientEvent('qb-gunrack:client:start_installing_gunrack', source)
end)
...
if Config.gunrack.use_keys_to_unlock_gunrack then
     ...
     QBCore.Functions.CreateUseableItem('keycuttingmachine', function(source, item)
          ...
          TriggerClientEvent('qb-gunrack:client:cut_key', source)
     end)
     ...
     RegisterNetEvent('qb-gunrack:server:start_cut_key', function(plate)
          local src = source
          local Player = QBCore.Functions.GetPlayer(src)
          ...
          TriggerClientEvent('qb-gunrack:client:keycuttingmachine_animation', src, plate)
     end)

     --
     RegisterNetEvent('qb-gunrack:server:cut_key', function(plate)
          local src = source
          local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-gunrack:server:create_gunrack', function(plate)
     local src = source
     local Player = QBCore.Functions.GetPlayer(src)
     ...
     TriggerClientEvent('qb-gunrack:client:open_gunrack', src, plate)
end)

RegisterNetEvent('qb-gunrack:server:open_gunrack', function(plate)
     local src = source
     local Player = QBCore.Functions.GetPlayer(src)
     if not Player then return end
     if not IsJobAllowed(Player.PlayerData.job.name) then
