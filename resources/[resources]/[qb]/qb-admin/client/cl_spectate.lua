...
function toggleSpectate(targetPed, targetPlayerId)
    ...
    if isSpectateEnabled then
        ...
        clearGamerTagInfo()
        DoScreenFadeIn(500)
        Mercy.Functions.Notify('Stopped spectating '..GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')')
        storedTargetPed = nil
    else
        ...
        DoScreenFadeIn(500)
        Mercy.Functions.Notify('Started spectating '..GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')')
...
function cleanupFailedResolve()
    ...
    Mercy.Functions.Notify('Stopped spectating, could not find player..', 'error')
end
...
RegisterNetEvent('Mercy/client/specPlayer', function(targetServerId, coords)
    ...
    if targetPlayerId == PlayerId() then
        return Mercy.Functions.Notify('You can\'t spectate yourself..', 'error')
    end
