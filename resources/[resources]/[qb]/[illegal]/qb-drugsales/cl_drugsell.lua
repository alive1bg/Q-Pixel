...
local function InteractPed(ped)
	...
	if percent < Config.ChanceSell then
		...
		TriggerEvent('qb-drugsales:notify', 'The buyer is calling the police!')
...
local function InitiateSales(entity)
	QBCore.Functions.TriggerCallback('qb-drugsales:server:GetCops', function(result)
		if result < Config.MinimumCops then
			TriggerEvent('qb-drugsales:notify', 'Buyer is not interested to buy now!')		
			...
			if isSoldtoPed then TriggerEvent('qb-drugsales:notify', 'You already spoke with this local') return false end
...
RegisterNetEvent('qb-drugsales:notify', function(msg)
