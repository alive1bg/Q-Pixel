...
function knockDoorAnim(home)
    ...
    if home then
        ...
        Wait(1000)
        dealerIsHome = true
        if Config.Dealers[currentDealer]["name"] == "grandma" then
            TriggerEvent("chatMessage", Lang:t("info.dealer_name", {dealerName = Config.Dealers[currentDealer]["name"]}), "info", Lang:t("info.grandma_man_knock_message"))
...
RegisterNetEvent('qb-drugs:client:usegrandmababy', function()
    local playerId = GetPlayerServerId(player)
    isHealingPerson = true
    QBCore.Functions.Progressbar("hospital_revive", Lang:t("info.reviving_player"), 5000, false, true, {
        disableMovement = false,
