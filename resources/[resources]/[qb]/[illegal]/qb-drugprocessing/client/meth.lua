...
local function ProcessChemicals()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processChemicals')
		...
		while timeLeft > 0 do
			...
			if #(GetEntityCoords(playerPed)-Config.CircleZones.MethProcessing.coords) > 2 then
				...
				TriggerServerEvent('qb-drugprocessing:cancelProcessing')
				break
			end
		end
...
local function ProcessTempUp()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_up"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processTempUp')
...
local function ProcessTempDown()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_down"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processTempDown')
...
local function ProcessProduct()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugprocessing:processMeth')
...
RegisterNetEvent('qb-drugprocessing:ProcessChemicals', function()
...
RegisterNetEvent('qb-drugprocessing:ChangeTemp', function()
		...
		QBCore.Functions.TriggerCallback('qb-drugprocessing:validate_items', function(result)
			if result then
				exports["thermite"]:OpenThermiteGame(6, 3, 5, 10,function() -- success
					...
					TriggerServerEvent('qb-drugprocessing:cancelProcessing')
					TriggerServerEvent('qb-drugprocessing:processFailUp')
				end)
			else
...
RegisterNetEvent('qb-drugprocessing:ChangeTemp2', function()
		...
		QBCore.Functions.TriggerCallback('qb-drugprocessing:validate_items', function(result)
			if result then
				exports["thermite"]:OpenThermiteGame(6, 3, 5, 10,function() -- success
					...
					TriggerServerEvent('qb-drugprocessing:cancelProcessing')
					TriggerServerEvent('qb-drugprocessing:processFailDown')
				end)
			else
...
RegisterNetEvent('qb-drugprocessing:ProcessProduct', function()
	local coords = GetEntityCoords(PlayerPedId(source))
...
RegisterNetEvent('qb-drugprocessing:EnterLab', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('qb-drugprocessing:ExitLab', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
