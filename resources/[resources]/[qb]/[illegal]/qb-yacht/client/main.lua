...
local function smashVitrine(k)
    ...
    QBCore.Functions.Progressbar("smash_vitrine", "Stocking a display", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        TriggerServerEvent('qb-yacht:server:setVitrineState', "isOpened", true, k)
        TriggerServerEvent('qb-yacht:server:setVitrineState', "isBusy", false, k)
        TriggerServerEvent('qb-yacht:server:vitrineReward')
        TriggerServerEvent('qb-yacht:server:setTimeout')
    ...
    TriggerServerEvent('qb-yacht:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('qb-yacht:client:setVitrineState', function(stateType, state, k)
...
CreateThread(function()
    local Dealer = AddBlipForCoord(Config.YachtLocation["coords"]["x"], Config.YachtLocation["coords"]["y"], Config.YachtLocation["coords"]["z"])
    SetBlipSprite (Dealer, 617)
    SetBlipDisplay(Dealer, 4)
    SetBlipScale  (Dealer, 0.0)
    SetBlipAsShortRange(Dealer, true)
    SetBlipColour(Dealer, 3)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Yacht")
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            for case,_ in pairs(Config.Locations) do
                ...
                local storeDist = #(pos - vector3(Config.YachtLocation["coords"]["x"], Config.YachtLocation["coords"]["y"], Config.YachtLocation["coords"]["z"]))
                if dist < 30 then
                    inRange = true
                    ...
                    if dist < 0.6 then
                        if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                            ...
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-yacht:server:getCops', function(cops)
