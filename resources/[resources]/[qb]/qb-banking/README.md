...
###### You do not need QB-moneysafe with this banking it has management_funds coded in and only players with specific job/gang and grade can see the business and gang account
...
### Using management_funds events to deposit money from other resources/scripts
```lua
TriggerEvent('qb-banking:management_funds:server:DepositMoney', src, AMOUNT , JOBNAME)
```
...
```lua
TriggerEvent('qb-banking:management_funds:server:DepositMoney', src, price , JOBNAME)
```
##### JOBNAME should be replace with the management_funds name if you want to deposit it to police management_funds account replace JOBNAME with 'police'
```lua
TriggerEvent('qb-banking:management_funds:server:DepositMoney', src, AMOUNT , 'police')
```
### Using the examples above the final triggerevent should look like this
```lua
TriggerEvent('qb-banking:management_funds:server:DepositMoney', src, price , 'police')
```
