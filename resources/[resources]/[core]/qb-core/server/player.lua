...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.cid = PlayerData.cid or QBCore.Player.CreateCid()
	...
	PlayerData.metadata['methknob1'] = PlayerData.metadata['methknob1'] or math.random(1, 100)
    PlayerData.metadata['methknob2'] = PlayerData.metadata['methknob2'] or math.random(1, 100)
    PlayerData.metadata['methknob3'] = PlayerData.metadata['methknob3'] or math.random(1, 100)
    -- Job
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        MySQL.Async.insert('INSERT INTO players (citizenid, cid, license, name, money, charinfo, job, gang, position, metadata, firstname, lastname) VALUES (:citizenid, :cid, :license, :name, :money, :charinfo, :job, :gang, :position, :metadata, :firstname, :lastname) ON DUPLICATE KEY UPDATE cid = :cid, name = :name, money = :money, charinfo = :charinfo, job = :job, gang = :gang, position = :position, metadata = :metadata, firstname = :firstname, lastname = :lastname', {
...
local playertables = {
    ...
    { table = 'phone_note' },
    { table = 'paychecks' },
    { table = 'oilrig_blender' },
    { table = 'oilrig_cdu' },
    { table = 'oilrig_position' },
    { table = 'oilrig_storage' },
    { table = 'player_boats' },
...
--[[function QBCore.Player.LoadInventory(PlayerData) -- for tnj-inventory
    PlayerData.items = {}
    local inventory = MySQL.Sync.prepare('SELECT inventory FROM players WHERE citizenid = ?', { PlayerData.citizenid })
    local missingItems = {}
    if inventory then
...
--[[function QBCore.Player.SaveInventory(source) -- for tnj-inventory
...
function QBCore.Player.CreateCid()
	local UniqueFound = false
	local Cid = nil

	while not UniqueFound do
		Cid = tostring(QBCore.Shared.RandomInt(3))
        local result = MySQL.Sync.prepare('SELECT COUNT(*) as count FROM players WHERE cid = ?', { Cid })
        if result == 0 then
            UniqueFound = true
        end
	end
...
-- PaycheckLoop() -- This just starts the paycheck system -- COMMENTED OUT TO REMOVE AUTO PAYCHECKS, NOW NEED TO GO PICK PAYCHECK UP FROM BANK
