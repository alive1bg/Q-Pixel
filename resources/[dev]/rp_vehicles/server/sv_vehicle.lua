RegisterNetEvent('rp_vehicles:server:ejectPassengers', function(pID, pVelocity)
	if pID ~= nil then
        if GetPlayerPed(pID) ~= nil then
            TriggerClientEvent('rp_vehicles:client:ejectPassengers', pID, pVelocity)
        end
    end
end)

QBCore.Functions.CreateCallback('rp_vehicles:server:fetchClass', function(souce, cb, plate)
	cb(nil)
